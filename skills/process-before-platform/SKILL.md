...
description: Clarify internal process, tooling, and automation requests before choosing a solution. Use only when the user explicitly invokes $process-before-platform to understand the work, test whether change is needed, compare practical options, expose ownership, and create a plain-language Markdown brief plus portable JSON. Do not use for customer-facing product discovery or as approval for initiatives, procurement, budgets, vendors, or architecture.
license: Apache-2.0
metadata:
